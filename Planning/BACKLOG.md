...
| OR-603 | ACTIVE | M6.2 | Population density & demographic distribution overlay | OR-601 | Overlay 3 visualizes resident density and income/age demographics across the tower, with non-colour encoding and drill-down to the cause-chain inspector |
| OR-604 | ACTIVE | M6.3 | Scrutiny external-pressure resource | OR-602 | Domain state and overlay-readable trend rise with expansion speed, inequality, unresolved crises, and aggressive policy; balanced response and service investment reduce it; high scrutiny modulates external-event pressure and can temporarily constrain expansion |
