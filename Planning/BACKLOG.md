...
| OR-704 | ACTIVE | M7.4 | Repair and validate volumetric window lighting | ART-005 | In `Tower`, each non-transit room has a visible, correctly layered warm window-light treatment; a Presentation test asserts the intended material/render state and a PlayMode screenshot verifies it is not hidden by room geometry. |
