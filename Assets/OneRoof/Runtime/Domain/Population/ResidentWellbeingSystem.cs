    ...
    public sealed class ResidentWellbeingSystem
    {
        ...
        public void Advance(PopulationState population, ElevatorBank elevatorBank, float serviceEfficiencyMultiplier = 1f)
        {
            ...
            foreach (var person in population.Persons)
            {
                ...
                var commute = Clamp(1f - (person.CurrentActivity == ActivityKind.Commuting ? 0.22f : 0f) - averageWait * 0.01f);
                var crowding = 0.82f;
                ...
                if (commute < GrievanceThreshold) grievances.Add("Long elevator waits are disrupting daily travel.");
                if (rent < GrievanceThreshold) grievances.Add("Household budget is under rent pressure.");
                if (noise < GrievanceThreshold) grievances.Add("Crowded travel is creating persistent noise stress.");
                ...
                person.Wellbeing.Update(satisfaction, strain, commute, crowding, noise, rent, service, events, grievances);
            }
        }
