...
namespace OneRoof.UI
{
    ...
    public static class StewardTheme
    {
        ...
        public static GUIStyle Panel => _panel ?? (_panel = Box(_surface ?? (_surface = Solid(Surface))));
        public static GUIStyle Recess => _recess ?? (_recess = Box(_ink ?? (_ink = Solid(Ink))));
        public static GUIStyle Button => _button ?? (_button = ButtonStyle(_raised ?? (_raised = Solid(Raised)), Text));
        public static GUIStyle ActiveButton => _activeButton ?? (_activeButton = ButtonStyle(_active ?? (_active = Solid(new Color(.18f, .42f, .36f))), Mint));
        public static GUIStyle WarningButton => _warningButton ?? (_warningButton = ButtonStyle(_warning ?? (_warning = Solid(new Color(.39f, .24f, .21f))), Coral));
        ...
        public static void Rule(float width)
        {
            ...
            GUI.DrawTexture(rect, _line ?? (_line = Solid(Line)));
        }
