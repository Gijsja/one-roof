...
### 2026-09-24 remediation and validation update

- Reviewed and completed the in-progress fixes for wellbeing affordability, diner routing, transit restore/reroute guards, business insolvency arithmetic, policy float normalization, room demolition cleanup, save-path authorization and atomic writes, overlay caching and floor matching, mode-session initialization, resident view culling and effect caching, authored room adoption, and procedural sprite cleanup. Added decree, treasury, business health, affordability, save round-trip, path privacy, overflow, and corrupt-save boundary tests.
- A full EditMode run exposed two additional failures: untouched morning-rush scrutiny settled at `0.8036` against the `<0.80` contract, and a business test assumed a new tenant with zero opening cash could pay payroll before customer receipts. Scrutiny mean reversion was adjusted; the business test now verifies wage arrears and the closed-loop balance prescribed by `Docs/12_ECONOMY.md`. A new overlay test was corrected to wait for business creation on tick 10.
- **Isolated worktree:** `/home/geisha/.codex/worktrees/audit-validation/one-roof` (managed worktree, retained for review). **Unity compile:** `/home/geisha/Unity/Hub/Editor/6000.3.24f1/Editor/Unity -batchmode -nographics -quit -projectPath <worktree> -logFile /tmp/one-roof-audit-compile.log` → exit `0`, no `error CS` lines.
- **Full EditMode:** `timeout 1500 <Unity> -batchmode -nographics -projectPath <worktree> -runTests -testPlatform EditMode -testResults /tmp/one-roof-audit-editmode-gate.xml -logFile /tmp/one-roof-audit-editmode-gate.log` → exit `0`, **494/494 passed**, 0 failed/skipped. This run includes the final same-process save/load/delete lock, which prevents deletion of an in-flight temporary file.
